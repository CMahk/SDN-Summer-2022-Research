...
cp -R ./mahk_config/heartbeat ../../config
...
for i in {1..3}; do
    ...
    docker container run --detach --name atomix-$i --hostname atomix-$i \
        --restart=always -v /mydata/config/heartbeat:/atomix $ATOMIX_IMAGE \
...
for i in {1..3}; do
    ...
    docker exec -i onos-$i /bin/bash -c "mkdir config; cat > config/cluster.json" < /mydata/config/heartbeat/cluster-$i.json
